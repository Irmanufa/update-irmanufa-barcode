...
function recordAttendance(memberId, method = "qr") {
  ...
  if (member.status !== "active") {
    ...
    playBeepWarning();
    showErrorResult(member, "ANGGOTA PASIF - TIDAK BISA ABSEN!");
    return false;
  }

  if (isAlreadyAttended(memberId)) {
    showAlreadyAttendedDialog(member);
    playBeepWarning();
    return false;
  }
  ...
  // Tampilkan hasil scan dengan tombol EDIT dan HAPUS
  showSuccessResult(member, attendance.id);

  Toast.success(`${member.name} berhasil absen!`, "Absensi Berhasil");
  return true;
}

// Tampilkan hasil scan SUKSES dengan tombol EDIT dan HAPUS
function showSuccessResult(member, attendanceId) {
  ...
  scanResultDiv.innerHTML = `
    <div style="text-align: center;">
      <i class="fas fa-check-circle" style="font-size: 48px; color: #059669; margin-bottom: 10px;"></i>
      <div class="member-name-large">${member.name}</div>
      <div class="member-info-grid">
        <span class="info-badge"><i class="fas fa-id-card"></i> ${member.code}</span>
        <span class="info-badge"><i class="fas fa-building"></i> ${member.division}</span>
        <span class="info-badge"><i class="fas fa-user-tag"></i> ${member.position || "Anggota"}</span>
        <span class="info-badge"><i class="fas fa-clock"></i> ${formatTime()}</span>
      </div>
      <div style="margin-top: 10px; font-size: 14px; color: #059669;">
        <i class="fas fa-check-circle"></i> ✓ ABSEN BERHASIL!
      </div>
    </div>
    <div class="scan-actions">
      <button class="btn btn-warning btn-sm" onclick="editAttendanceById(${attendanceId})">
        <i class="fas fa-edit"></i> Edit Data Absen
      </button>
      <button class="btn btn-danger btn-sm" onclick="deleteAttendanceById(${attendanceId})">
        <i class="fas fa-trash"></i> Hapus Absen
      </button>
    </div>
  `;
}

// Tampilkan hasil scan ERROR (QR tidak valid)
function showErrorResult(member, errorMessage) {
  const scanResultDiv = document.getElementById("scanResult");
  if (!scanResultDiv) return;
  ...
  scanResultDiv.innerHTML = `
    <div style="text-align: center;">
      <i class="fas fa-times-circle" style="font-size: 48px; color: #ef4444; margin-bottom: 10px;"></i>
      <div class="member-name-large" style="color: #991b1b;">${member ? member.name : "QR Code Tidak Valid!"}</div>
      <div class="member-info-grid">
        ${member ? `<span class="info-badge"><i class="fas fa-id-card"></i> ${member.code}</span>` : ''}
        ${member ? `<span class="info-badge"><i class="fas fa-building"></i> ${member.division}</span>` : ''}
      </div>
      <div style="margin-top: 10px; font-size: 16px; font-weight: bold; color: #991b1b;">
...
// Tampilkan dialog untuk anggota yang SUDAH ABSEN hari ini
function showAlreadyAttendedDialog(member) {
  const todayAttendance = AppState.attendance.find(a => 
    a.memberId === member.id && new Date(a.timestamp).toDateString() === new Date().toDateString()
  );
  ...
  scanResultDiv.innerHTML = `
    <div style="text-align: center;">
      ...
      <div class="member-name-large" style="color: #92400e;">${member.name}</div>
      <div class="member-info-grid">
        <span class="info-badge"><i class="fas fa-id-card"></i> ${member.code}</span>
        <span class="info-badge"><i class="fas fa-building"></i> ${member.division}</span>
        <span class="info-badge"><i class="fas fa-clock"></i> ${todayAttendance?.time || ""}</span>
      </div>
      <div style="margin-top: 10px; font-size: 16px; font-weight: bold; color: #92400e;">
        ⚠️ SUDAH ABSEN HARI INI!
      </div>
    </div>
    <div class="scan-actions">
      <button class="btn btn-warning btn-sm" onclick="editAttendanceById(${todayAttendance?.id})">
        <i class="fas fa-edit"></i> Edit Data Absen
      </button>
      <button class="btn btn-danger btn-sm" onclick="deleteAttendanceById(${todayAttendance?.id})">
        <i class="fas fa-trash"></i> Hapus & Absen Ulang
      </button>
    </div>
  `;
}

// Tampilkan QR Code tidak dikenal
function showUnknownQRResult() {
  const scanResultDiv = document.getElementById("scanResult");
  if (!scanResultDiv) return;
  ...
  scanResultDiv.innerHTML = `
    <div style="text-align: center;">
      <i class="fas fa-times-circle" style="font-size: 48px; color: #ef4444; margin-bottom: 10px;"></i>
      <div style="font-size: 20px; font-weight: bold;">QR Code Tidak Valid!</div>
      <div class="member-info-grid">
        <span class="info-badge"><i class="fas fa-qrcode"></i> QR Code tidak dikenali</span>
      </div>
      <div style="margin-top: 10px; font-size: 14px; color: #64748b;">
        Pastikan QR Code adalah milik anggota IRMANUFA
      </div>
    </div>
  `;
}
  ...
  modal.innerHTML = `
    <div class="modal">
      ...
      <div class="input-group">
        ...
        <input type="text" value="${attendance.memberName}" disabled style="background:#f1f5f9;">
      </div>
      <div class="input-group">
        <label>Divisi</label>
        <input type="text" value="${attendance.division}" disabled style="background:#f1f5f9;">
      </div>
      <div class="input-group">
      ...
      <div class="input-group">
        ...
        <input type="text" id="editKegiatan" value="${attendance.kegiatan || AppState.currentKegiatan}">
      </div>
      <div class="modal-buttons">
        <button class="btn btn-secondary" onclick="this.closest('.modal-overlay').remove()">Batal</button>
        <button class="btn btn-primary" onclick="saveEditAttendance(${attendance.id})">Simpan Perubahan</button>
      </div>
    </div>
  `;
...
function saveEditAttendance(attendanceId) {
  ...
  // Update tampilan scan result
  const updatedAttendance = AppState.attendance.find(a => a.id == attendanceId);
  if (updatedAttendance) {
    const member = getMemberById(updatedAttendance.memberId);
    showSuccessResult(member, attendanceId);
  }
}

// ==================== QR SCANNER (RESPONS CEPAT) ====================
async function startScanner() {
  ...
  // Konfigurasi untuk respons lebih cepat
  const config = {
    fps: 30,
  ...
  html5QrCode.start({ facingMode: "environment" }, config, (decodedText) => {
    ...
    container.innerHTML = '<div class="empty-state"><i class="fas fa-exclamation-triangle"></i><p>Gagal mengakses kamera! Pastikan izin kamera diberikan.</p><button class="btn btn-primary" style="margin-top: 10px;" onclick="startScanner()"><i class="fas fa-redo"></i> Coba Lagi</button></div>';
...
function stopScanner() {
  if (AppState.qrScanner) {
    ...
    document.getElementById("reader").innerHTML = '<div class="empty-state"><i class="fas fa-qrcode"></i><p>Scanner siap. Klik "Mulai Scan"</p></div>';
...
function generateMemberQR() {
  ...
  if (member.status !== "active") {
    Toast.warning(`Anggota ${member.name} berstatus PASIF! Tidak bisa generate QR.`);
    return;
  }
  ...
  QRCode.toCanvas(document.createElement("canvas"), qrData, { width: 200, margin: 2, color: { dark: "#059669", light: "#ffffff" } }, function(error, canvas) {
    if (error) {
...
function downloadQRAsPNG() {
  ...
  const qrSize = 300, padding = 30;
  const width = qrSize + padding * 2, height = qrSize + 250;
...
function downloadQRAsPDF() {
  ...
  doc.setFontSize(10);
  doc.text("Ikatan Remaja Masjid Jami Nurul Falah", 105, 25, { align: "center" });
  ...
  modal.innerHTML = `
    <div class="modal" style="max-width: 400px;">
      ...
      <div class="modal-buttons" style="justify-content: center; gap: 16px;">
        ...
        <button class="btn btn-danger" onclick="confirmDeleteMember('${id}')" style="padding: 10px 24px;">Hapus</button>
      </div>
    </div>
  `;
  document.body.appendChild(modal);
...
function renderMemberTable(searchTerm = "") {
  ...
  if (searchTerm) {
    members = members.filter(m => m.name.toLowerCase().includes(searchTerm.toLowerCase()) || m.code.toLowerCase().includes(searchTerm.toLowerCase()) || m.division.toLowerCase().includes(searchTerm.toLowerCase()));
  }
...
function showWhatsAppModal() {
  Toast.info("Fitur WhatsApp akan segera hadir!");
}

function resetAllData() {
  showConfirmModal("Hapus Semua Data Absensi", "Apakah Anda yakin ingin menghapus SEMUA data absensi?", () => {
    AppState.attendance = [];
    saveAttendance();
    updateAllDisplays();
...
function resetMembersToDefault() {
  showConfirmModal("Reset Data Anggota", "Apakah Anda yakin ingin mereset SEMUA data anggota ke default?", () => {
...
function showConfirmModal(title, message, onConfirm) {
  ...
  modal.innerHTML = `
    <div class="modal" style="max-width: 400px;">
      ...
      <div class="modal-buttons" style="justify-content: center; gap: 16px;">
        <button class="btn btn-secondary" onclick="this.closest('.modal-overlay').remove()" style="padding: 10px 24px;">Batal</button>
        <button class="btn btn-danger" id="confirmDeleteBtn" style="padding: 10px 24px;">Ya, Hapus</button>
      </div>
    </div>
  `;
  document.body.appendChild(modal);
...
function showSection(section) {
  ...
  if (section === "generator") {
    updateMemberSelect();
    const qrContainer = document.getElementById("qrCodeDisplay");
    if (qrContainer) qrContainer.innerHTML = '<div class="empty-state"><i class="fas fa-qrcode" style="font-size: 80px; color: #94a3b8;"></i><p style="margin-top:10px;">Pilih anggota dan klik Generate</p></div>';
    const infoContainer = document.getElementById("memberInfoDisplay");
...
function logout() {
  showConfirmModal("Keluar", "Apakah Anda yakin ingin keluar dari sistem?", () => {
    ...
    <div class="dashboard">
      <nav class="navbar">
        <div class="nav-content">
          <div class="nav-brand">
            ...
            <div><h1>IRMANUFA QR Absensi</h1><small>Kabinet Golden Generation</small></div>
          </div>
          <div class="nav-stats">
            <span class="stat-badge"><i class="fas fa-users"></i> ${activeCount} Anggota Aktif</span>
            <span class="stat-badge"><i class="fas fa-calendar-check"></i> ${stats.todayCount} Hadir Hari Ini</span>
            <span class="stat-badge"><i class="fas fa-user-clock"></i> ${passiveCount} Pasif</span>
          </div>
      ...
      <div class="sidebar">
        <div class="sidebar-header">
          ...
          <div class="user-info">
            <h3>Admin IRMANUFA</h3>
            <p>Administrator Sistem</p>
            <div class="user-badge"><i class="fas fa-shield-alt"></i> Super Admin</div>
          </div>
        </div>
        <div class="sidebar-menu">
          ...
          <a href="#" class="menu-item" data-section="scanner" onclick="showSection('scanner')"><i class="fas fa-qrcode"></i><span>Scan QR Code</span><span class="menu-badge">New</span></a>
      ...
      <div class="main-content">
        <div class="welcome-card">
          <div class="welcome-text">
            ...
            <p>Sistem Absensi Digital Berbasis QR Code IRMANUFA</p>
            <small><i class="fas fa-qrcode"></i> Scan QR Code untuk absensi cepat | <i class="fas fa-check-circle"></i> Hanya anggota AKTIF yang bisa absen</small>
          </div>
          <div class="date-info">
        ...
        <section id="dashboardSection" class="content-section active">
          <div class="stats-grid">
            <div class="stat-card"><div class="stat-icon"><i class="fas fa-users"></i></div><div class="stat-number" id="activeMembers">${stats.activeMembers}</div><div class="stat-label">Anggota Aktif</div></div>
            <div class="stat-card"><div class="stat-icon"><i class="fas fa-calendar-day"></i></div><div class="stat-number" id="todayCount">${stats.todayCount}</div><div class="stat-label">Hadir Hari Ini</div></div>
            <div class="stat-card"><div class="stat-icon"><i class="fas fa-chart-line"></i></div><div class="stat-number" id="totalAttendance">${stats.totalAttendance}</div><div class="stat-label">Total Absensi</div></div>
          </div>
          ...
          <div class="card"><div class="card-header"><i class="fas fa-chart-pie"></i><h3>Persentase Kehadiran per Divisi</h3><small style="color:#64748b;">(Warna berbeda setiap divisi)</small></div><div class="chart-wrapper"><canvas id="divisionChart"></canvas></div></div>
        ...
        <section id="scannerSection" class="content-section">
          ...
          <div class="info-card"><i class="fas fa-info-circle"></i> <strong>Cara Penggunaan:</strong><ol><li>Klik "Mulai Scan"</li><li>Izinkan akses kamera</li><li>Arahkan ke QR Code anggota</li><li>Nama anggota akan muncul dengan JELAS dan BESAR</li><li>Absensi otomatis tercatat dengan suara BEEP</li><li>Klik Flash untuk menyalakan lampu kamera</li><li>Jika salah scan, bisa langsung EDIT atau HAPUS</li></ol></div>
        </section>
        ...
        <section id="reportsSection" class="content-section">
          <div class="card">
            ...
            <div class="kegiatan-input"><label><i class="fas fa-tag"></i> Nama Kegiatan:</label><input type="text" id="kegiatanName" placeholder="Contoh: Rapat Koordinasi, Kajian Rutin, dll"></div>
    ...
    <div class="login-container">
      <div class="login-card">
        ...
        <form onsubmit="return handleLogin(event)">
          <div class="input-group"><label><i class="fas fa-user"></i> Username</label><input type="text" id="username" placeholder="admin" required></div>
          <div class="input-group"><label><i class="fas fa-lock"></i> Password</label><div class="password-container"><input type="password" id="password" placeholder="••••••••" required><button type="button" class="toggle-password" onclick="togglePassword()"><i class="fas fa-eye"></i></button></div></div>
        ...
        <div class="login-footer"><p>IRMANUFA Kabinet Golden Generation 2027-2029</p></div>
      </div>
    </div>
  `;
}
